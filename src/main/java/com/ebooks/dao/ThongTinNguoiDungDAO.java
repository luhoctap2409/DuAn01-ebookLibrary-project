...
public class ThongTinNguoiDungDAO {
    ...
    public void insert(ThongTinNguoiDung entity) {
        String sql = "INSERT INTO ThongTinNguoiDung (tenNguoiDung, gioiTinh, soDienThoai, email, anhDaiDien,tenDangNhap, moTaCaNhan) VALUES ( ?, ?, ?, ?, ?,?,?)";
    ...
    public void update(ThongTinNguoiDung entity) {
        String sql = "UPDATE dbo.ThongTinNguoiDung SET tenNguoiDung = ?, gioiTinh = ?, soDienThoai = ?, email = ?, anhDaiDien = ? , tenDangNhap=? , moTaCaNhan=? WHERE maNguoiDung = ?";
        JdbcHelper.executeUpdate(sql,
                entity.getTenNguoiDung(),
                entity.isGioiTinh(),
                entity.getSoDienThoai(),
    ...
    public void delete(String id) {
        String sql = "DELETE FROM ThongTinNguoiDung WHERE MaNguoiDung=?";
        JdbcHelper.executeUpdate(sql, id);
    }
    
    public void deleteND(String id) {
        String sql = "DELETE FROM ThongTinNguoiDung WHERE tenDangNhap=?";
        JdbcHelper.executeUpdate(sql, id);
    }
    ...
    public List<ThongTinNguoiDung> selectAllNotACC() {
        String sql = "SELECT * FROM ThongTinNguoiDung WHERE maNguoiDung NOT IN (SELECT maNguoiDung FROM TaiKhoan)";
        return select(sql);
    }
    ...
    public ThongTinNguoiDung findById(String id) {
        String sql = "SELECT * FROM ThongTinNguoiDung WHERE maNguoiDung=?";
        List<ThongTinNguoiDung> list = select(sql, id);
        return list.size() > 0 ? list.get(0) : null;
    }
    
    public ThongTinNguoiDung findByIdND(String id) {
        String sql = "SELECT * FROM ThongTinNguoiDung WHERE tenDangNhap=?";
        List<ThongTinNguoiDung> list = select(sql, id);
        return list.size() > 0 ? list.get(0) : null;
    }
}
